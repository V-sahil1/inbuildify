...
async function createUser(currentUser, body, files) {
  ...
  // Make login ID optional for "contact" (client with no login)
  if (has_login === "true" && !login_id) {
    throw {
      status: 400,
      message: "Login ID is required for users with login access.",
    };
  }
  ...
  if (has_login === "false") {
    // Contact without login – no password
    finalPassword = null;
  } else {
  ...
  if (email_login_credentials === "true" && has_login === "true") {
