              ...
              json_build_object(
                ...
                'quotation_version_items', (
                  SELECT COALESCE(json_agg(json_build_object(
                    ...
                    'extra_item', COALESCE(qvi.extra_item, false),
                    'extra_type', qvi.extra_type,
    ...
    try {
      ...
      let query = `
        SELECT qv.quotation_version_id, qv.quotation_id, q.reference_number, qv.quotation_version_no,
          ...
          (SELECT COALESCE(json_agg(json_build_object('quotation_version_item_id', qvi.quotation_version_item_id, 'price_list_item_id', qvi.price_list_item_id, 'price_list_item_description', qvi.price_list_item_description, 'price_list_item_cost', qvi.price_list_item_cost, 'quantity', qvi.quantity, 'total_price', qvi.total_price, 'package_id', qvi.package_id, 'extra_item', COALESCE(qvi.extra_item, false), 'extra_type', qvi.extra_type, 'is_price_list_item_cost_mismatch', CASE WHEN qvi.price_list_item_id IS NOT NULL AND pli.cost IS NOT NULL AND qvi.price_list_item_cost::numeric != pli.cost::numeric THEN true ELSE false END, 'is_package_cost_mismatch', CASE WHEN qvi.package_id IS NOT NULL AND p.cost IS NOT NULL AND qvi.package_cost::numeric != p.cost::numeric THEN true ELSE false END, 'is_system_data', qvi.price_list_item_is_system_data, 'is_automatically_mapped', CASE WHEN fppim_items.price_list_item_id IS NOT NULL THEN true ELSE false END)), '[]'::json) FROM quotation_version_items qvi LEFT JOIN price_list_item pli ON qvi.price_list_item_id = pli.price_list_item_id LEFT JOIN package p ON qvi.package_id = p.package_id LEFT JOIN floor_plan_pricelist_item_map fppim_items ON qv.floor_plan_id = fppim_items.floor_plan_id AND qvi.price_list_item_id = fppim_items.price_list_item_id WHERE qvi.quotation_version_id = qv.quotation_version_id) as quotation_version_items,
