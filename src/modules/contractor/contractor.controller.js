...
export async function createContractor(req, res) {
  ...
  try {
    ...
    `;
    const existingContractorResult = await client.query(
      existingContractorQuery,
      [lowerCaseEmail, builderId],
    );
    ...
    if (existingContractorResult.rows.length > 0) {
      return errorResponse(
        res,
        409,
        "Contractor with this email already exists for this builder.",
      );
    }
    ...
    if (error.code === "23505") {
      return errorResponse(
        res,
        409,
        "Contractor with this email already exists.",
      );
    }
...
export async function updateContractor(req, res) {
  ...
  try {
    ...
    if (updates.service) {
      ...
      if (serviceResult.rowCount === 0) {
        return errorResponse(
          res,
          400,
          `Service '${updates.service}' not found.`,
        );
      }
    ...
    const serviceName =
      serviceResult.rowCount > 0 ? serviceResult.rows[0].service : null;
