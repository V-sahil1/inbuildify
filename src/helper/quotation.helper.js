...
export const syncCompactionReportCharge = async (leadsId, builderId, companyId, userId, client) => {
  try {
    const { Leads, PropertyDetail, Quotation, QuotationVersion, QuotationVersionItem, PriceList, PriceListItem } = db;

    // 1. Fetch property details via Lead
    const lead = await Leads.findOne({
      ...
      transaction: client?.transaction, // Note: client might be pg-client, Sequelize needs Sequelize transaction
    });
    ...
    // 2. Fetch all non-approved quotation versions for the lead
    ...
    if (status === "not_available" && provider === "builder") {
      // 3. Ensure the master PriceListItem exists
      ...
      // REMOVE charge from all non-approved versions
