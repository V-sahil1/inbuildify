...
exports.createJob = async (req, res) => {
  ...
  try {
    ...
    const checkLeadExists = await client.query(`SELECT 1 FROM leads WHERE lead_id = $1 AND builder_id = $2 AND decision is not null AND quotation_version_id is not null`, [lead_id, builderId]);
    if (checkLeadExists.rows.length > 0) {
      await client.query("ROLLBACK");
      return errorResponse(res, 404, "Lead already converted to job or lost status.");
    }
    ...
    if (status === "WON") {
      ...
      `, [quotation_version_id, quotationResult.rows.map(row => row.quotation_id)]);
      
      if (quotationVersionExists.rows.length === 0) {
        await client.query("ROLLBACK");
        return errorResponse(res, 404, "Quotation version not found for that quotation.");
      }
      ...
      if (version_number !== latestVersion) {
        await client.query("ROLLBACK");
        return errorResponse(res, 400, `Only the latest quotation version (v${latestVersion}) can be used when status is WON`);
      }
